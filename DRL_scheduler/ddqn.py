...
def nature_cnn(observation_space):
    ...
    out = nn.Sequential(cnn, nn.Linear(n_flatten, final_layer),
        nn.ReLU(),)

    return out, final_layer
...
class Network(nn.Module):
    def __init__(self, env, obs, device, double=True):
        ...
        conv_net, hidden_nodes = nature_cnn(obs)

        self.net = nn.Sequential(conv_net, nn.Linear(hidden_nodes, self.num_actions))
